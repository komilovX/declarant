  ...
  <div>
    ...
    <el-col :span="24" :md="16" :sm="24" :offset="4">
      <el-table border :data="serviceList" size="mini">
        ...
        >
          <template slot-scope="{ row: {total_price, currency}}">
    ...
    serviceList() {
      let service = this.services
        .map(({id, name, number, total_price, comment, currency}) =>
          ({id, name, number, total_price, currency, comment, type: 'service'} ))
      let declarant = this.declarant_documents
        .filter(d => d.status == 'finish')
        .map(({id, name, number, total_price, comment, currency}) =>
          ({id, name, number, total_price, currency, comment, type: 'declarant' } ))
